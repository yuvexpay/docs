...
When the customer pays, your endpoint receives a `POST` whose
`X-Webhook-Event` header is `PAYMENT_PAID`. The body is the event itself —
there is no envelope, and the event name is **not** in the body:
...
Switch on `X-Webhook-Event`, and always
[verify the webhook signature](/guides/webhooks#verifying-signatures) before
acting on the event.
