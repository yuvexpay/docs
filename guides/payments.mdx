...
The first time you split to a company, that company has to accept the
relationship in its dashboard. Until it does, creating the payment fails with
`SPLIT_RELATIONSHIP_PENDING`. Once it has accepted, later payments carry no
further friction.
...
an individual, KYC and KYB for a business, on both sides. The check runs when
the payment is created, not when it settles. Your own company failing it returns
`403 SPLIT_ORIGINATOR_NOT_APPROVED`; a recipient failing it returns
...
production payment only to a production company. The other direction returns
`400 SPLIT_ENVIRONMENT_MISMATCH`, never a silent no-op. Sandbox splits never
touch a production balance, statement or report.
