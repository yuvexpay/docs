...
by default and counted per environment, and a limited number of active partners
(three by default). Beyond either, the request is rejected —
`SPLIT_RELATIONSHIP_REQUEST_LIMIT` for outstanding invites,
`SPLIT_PARTNER_LIMIT_EXCEEDED` for active partners.
