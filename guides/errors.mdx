...
| `SPLIT_RECIPIENT_LIMIT_EXCEEDED` | 400 | `splits` carries more entries than the recipient limit configured for your account (three by default). |
| `SPLIT_SELF_RECIPIENT` | 400 | A split names your own company as the recipient. |
| `SPLIT_ORIGINATOR_NOT_APPROVED` | 403 | Your own company has not completed KYC (and KYB, for a business), so it cannot originate split payments. |
| `SPLIT_AMOUNT_EXCEEDS_NET` | 400 | The splits total more than `netAmount`. The message names the overflow; nothing is clamped. |
| `SPLIT_RELATIONSHIP_PENDING` | 400 | The recipient has not accepted split payments from your company yet. This is the single answer for every recipient that cannot receive a split yet, apart from a pair that declined or was revoked — both need an invitation your company already sent — so it reads the same whether or not the company ID exists and cannot be used to probe for company IDs. |
| `SPLIT_RELATIONSHIP_REQUEST_LIMIT` | 400 | Your company already has the maximum number of split requests awaiting an answer (20 by default, per environment). Wait for answers before naming new recipients. |
| `SPLIT_RELATIONSHIP_REVOKED` | 400 | The relationship was revoked, by either side. Revocation is forward-only: payments created before it still settle. |
| `SPLIT_RELATIONSHIP_DECLINED` | 400 | The recipient refused your company's invitation to receive splits. `details.recipients` names the company IDs that declined. Send a new invitation from the dashboard to ask again. |
| `SPLIT_RECIPIENT_INACTIVE` | 400 | A recipient that had accepted the relationship is no longer active. |
| `SPLIT_RECIPIENT_NOT_APPROVED` | 400 | A recipient that had accepted the relationship no longer meets KYC (and KYB, for a business). Approval is checked when the payment is created. |
| `SPLIT_ENVIRONMENT_MISMATCH` | 400 | A recipient that had accepted the relationship is in the other environment. |
| `ANTICIPATION_SPLIT_PAYMENT` | 400 | A payment that carries splits cannot be anticipated. |
...
| `PARTIAL_REFUND_NOT_SUPPORTED` | 400 | A partial refund needs a settled payment and a routing provider that supports partial refunds; this payment fails one of the two. Send no `amount`, or the full remaining refundable amount. |
...
| `INSUFFICIENT_BALANCE_FOR_REFUND` | 400 | Your available balance is not sufficient to issue this refund. On a payment with [splits](/guides/payments#splits), every party is debited in proportion to what it received and this check covers the originating company's share only — a recipient can go negative and never blocks the refund. |
