...
| `SPLIT_RELATIONSHIP_PENDING` | 400 | The recipient has not accepted split payments from your company yet. This is the single answer for every recipient that cannot receive a split yet, whether or not the company ID exists, so it cannot be used to probe for company IDs. |
...
| `SPLIT_PARTNER_LIMIT_EXCEEDED` | 400 | The originating company already has as many active split partners as its recipient limit allows (three by default). |
