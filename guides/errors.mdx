...
| `SPLIT_RECIPIENT_NOT_FOUND` | 400 | `recipientCompanyId` does not match a YuvexPay company. |
| `SPLIT_RECIPIENT_INACTIVE` | 400 | The recipient company exists but is not active. |
| `SPLIT_RECIPIENT_NOT_APPROVED` | 400 | The recipient has not completed KYC (and KYB, for a business). Approval is checked when the payment is created. |
...
| `SPLIT_ENVIRONMENT_MISMATCH` | 400 | A sandbox payment names a production recipient, or a production payment names a sandbox one. |
| `SPLIT_AMOUNT_EXCEEDS_NET` | 400 | The splits total more than `netAmount`. The message names the overflow; nothing is clamped. |
| `SPLIT_RELATIONSHIP_PENDING` | 400 | The recipient has not accepted split payments from your company yet. |
