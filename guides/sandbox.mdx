...
- **Splits** stay inside one environment. A sandbox payment can only split to a
  ...
  company; the other direction is rejected with
  `400 SPLIT_ENVIRONMENT_MISMATCH`, never silently ignored. A sandbox split
  never touches a production balance, statement or report.
